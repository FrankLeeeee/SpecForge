...
class LlamaExperts(nn.Module):
    def __init__(self, config):
        ...
        self.expert_ratio = config.expert_ratio

        if self.expert_ratio is not None:
            for idx, frac in enumerate(self.expert_ratio):
                self.register_parameter(
                    f"gate_up_proj_{idx}",
                    nn.Parameter(
                        torch.empty(
                            1, self.hidden_size, 2 * int(self.expert_dim * frac)
                        )
                    ),
                )
                self.register_parameter(
                    f"down_proj_{idx}",
                    nn.Parameter(
                        torch.empty((1, int(self.expert_dim * frac), self.hidden_size))
                    ),
                )
        else:
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.expert_ratio is not None:
            ...
            hidden_states_chunks = hidden_states.chunk(len(self.expert_ratio), dim=0)
            for idx, _ in enumerate(self.expert_ratio):
                gate_up_proj = getattr(self, f"gate_up_proj_{idx}")
                down_proj = getattr(self, f"down_proj_{idx}")
                hidden_states_for_current_expert = hidden_states_chunks[idx]
                gate_up = torch.bmm(hidden_states_for_current_expert, gate_up_proj)
                gate, up = gate_up.chunk(2, dim=-1)  # not supported for DTensors
                next_states = torch.bmm((up * self.act_fn(gate)), down_proj).view(
                    -1, self.hidden_size
                )
                output_states.append(next_states)
            next_states = torch.cat(output_states, dim=0).view(-1, self.hidden_size)
        else:
...
class LlamaForCausalLMEagle3MoE(Eagle3DraftModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            if module.expert_ratio is not None:
                for idx, _ in enumerate(module.expert_ratio):
                    gate_up_proj = getattr(module, f"gate_up_proj_{idx}")
                    down_proj = getattr(module, f"down_proj_{idx}")
                    gate_up_proj.data.normal_(mean=0.0, std=std)
                    down_proj.data.normal_(mean=0.0, std=std)
            else:
