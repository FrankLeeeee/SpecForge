...
from specforge.modeling.draft.llama3_eagle_moe import LlamaForCausalLMEagle3MoE

# from model_module import LlamaForCausalLMEagle3


class TestLlamaForCausalLMEagle3MoELoading(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = Llama4TextConfig(**config_dict)

    def tearDown(self):
        shutil.rmtree(self.temp_dir)

    # def test_model_initialization_with_shared_expert(self):
    #     self.config.has_shared_expert = True
    #     model = LlamaForCausalLMEagle3MoE(self.config)

    #     self.assertIsInstance(model.midlayer.self_attn, LlamaAttention)
    #     self.assertIsInstance(model.midlayer.moe_layer, LlamaMoeLayer)
    #     self.assertIsInstance(model.midlayer.moe_layer.experts, LlamaExperts)
    #     self.assertIsInstance(model.midlayer.moe_layer.router, LlamaRouter)
    #     self.assertIsInstance(model.midlayer.moe_layer.shared_expert, LlamaMLP)
    ...
    # def test_model_initialization_without_shared_expert(self):
    ...
    #     self.assertIsInstance(model.midlayer.moe_layer.experts, LlamaExperts)
    #     self.assertIsInstance(model.midlayer.moe_layer.router, LlamaRouter)
    #     self.assertEqual(model.midlayer.moe_layer.shared_expert, None)
    #     self.assertIsInstance(model.midlayer.hidden_norm, LlamaRMSNorm)
    #     self.assertIsInstance(model.midlayer.input_layernorm, LlamaRMSNorm)
    #     self.assertIsInstance(model.midlayer.post_attention_layernorm, LlamaRMSNorm)
    #     self.assertEqual(model.midlayer.hidden_size, self.config.hidden_size)
...
if __name__ == "__main__":
    ...
    suite.addTest(unittest.makeSuite(TestLlamaForCausalLMEagle3MoELoading))
